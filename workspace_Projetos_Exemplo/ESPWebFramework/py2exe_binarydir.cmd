...
@set prgname="binarydir"

if exist %pythondir% (
  if not exist "%pythondir%\Scripts\pip.exe" (
    copy /Y C:\Espressif\utils\get-pip.py %pythondir%
  )
  ...
  if exist "python.exe" (
    ...
    if exist "%pythondir%\Scripts\pip.exe" (
      cd Scripts\
      if not exist "%pythondir%\Lib\site-packages\slimmer\__init__.py" (
        echo Installing slimmer...
        pip.exe install slimmer
      )
    ...
    if exist "%pythondir%\dist\%prgname%.exe" (
      ...
      copy /Y dist\_socket.pyd %curdir%
